...
<body>
  ...
  <div class="tab-buttons">
    ...
    <button onclick="sendPaymentReminders()" style="background-color: #ffc107; color: #000;">Send Payment Reminders</button>
  </div>
  ...
  <script>
    ...
    function sendPaymentReminders() {
      if (confirm('Send payment reminders to all clients whose payments are due soon or overdue?')) {
        fetch('/send_payment_reminders')
          .then(response => response.json())
          .then(data => {
            alert(data.message);
          })
