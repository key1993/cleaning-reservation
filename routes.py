        ...
        if user_email:
            ...
            if client_doc:
                ...
                if existing_client:
                    ...
                    # Truly new client: create minimal record so they appear in admin
...
@routes.route("/api/client_device_reset_status", methods=["GET"])
def client_device_reset_status():
    """
    For the mobile app: check if this client has an active one-time pass (after admin reset device).
    If true, the app can allow one-time login from the current device; the next update_fcm_token will consume the pass.
    """
    try:
...
@routes.route("/api/applepay/config", methods=["GET"])
def apple_pay_config():
    """
    Returns the amount (in JOD) the mobile app should present in the native Apple Pay
    sheet for this client's subscription. Does not create a transaction — Apple Pay
    generates the payment token client-side first; see api/applepay/process_subscription_payment.
    """
    try:
        user_email = request.args.get("user_email")
        ...
        subscription_type = client.get("subscription_type", "monthly")
        amount = (
            APPLE_PAY_YEARLY_AMOUNT_JOD
            if subscription_type == "yearly"
            else APPLE_PAY_MONTHLY_AMOUNT_JOD
        )
        ...
        return jsonify({
            ...
            "amount": amount,
            "currency": "JOD",
            "label": "Smart Solar Subscription",
        }), 200
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500


@routes.route("/api/applepay/process_subscription_payment", methods=["POST"])
def apple_pay_process_subscription_payment():
    """
    Receives the PKPaymentToken produced by the native Apple Pay sheet.
...
# Admin panel route moved to admin.py blueprint for proper authentication
...
@routes.route("/reset_client_device/<client_id>", methods=["POST"])
def reset_client_device(client_id):
    """
    ...
    try:
        ...
        # Also clear from users collection if same email exists
        if client_email:
...
# Firebase test endpoint moved to admin.py blueprint
