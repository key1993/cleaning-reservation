...
def client_device_health():
    ...
    try:
        ...
        ha_token = (data.get("ha_token") or "").strip() or request.args.get("ha_token", "").strip()
        if not ha_token:
            auth = request.headers.get("Authorization", "") or ""
            if auth.lower().startswith("bearer "):
                ha_token = auth[7:].strip()

        if not ha_token:
            return jsonify({
                "success": False,
                "error": "ha_token is required (JSON body, query ?ha_token=, or Authorization: Bearer <token>)",
            }), 400

        client = clients_collection.find_one({"ha_token": ha_token})
        if not client:
            return jsonify({"success": False, "error": "Client not found for this token"}), 404
        ...
        if inverter_ok is not None:
            update_doc["health_inverter_ok"] = inverter_ok
